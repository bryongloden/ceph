...
bool RGWLC::LCWorker::should_work(utime_t& now)
{
  ...
  if (cct->_conf->rgw_lc_debug_interval > 0) {
	  /* We're debugging, so say we can run */
	  return true;
  } else if ((bdt.tm_hour*60 + bdt.tm_min >= start_hour*60 + start_minite) ||
...
int RGWLC::LCWorker::schedule_next_start_time(utime_t& now)
{
  int start_hour;
  int start_minite;
  int end_hour;
  ...
  if (cct->_conf->rgw_lc_debug_interval > 0) {
	  return (cct->_conf->rgw_lc_debug_interval);
  }
